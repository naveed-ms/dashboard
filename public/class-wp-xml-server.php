...
class wp_xmlrpc_server extends IXR_Server {
	...
	public function __construct() {
		$this->methods = array(
            ...
            'wp.getPostByName' => 'this:wp_getPostByName',
			'wp.getMyPosts'=>'this:wp_getMyPosts',
			'wp.getPostFor'=>'this:wp_getPostFor',
			// Blogger API
...
/**
	 ...
	 * @website https://github.com/shakirmengrani
	 */

	public function wp_getPostByName($args){
		$username = $args[1];
		$password = $args[2];
		if ( ! $user = $this->login( $username, $password ) )
			return $this->error;
		$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields','page' ), 'wp.getPostByName' );
		do_action( 'xmlrpc_call', 'wp.getPostByName' );
		$p = array(
			'name'        => $args[3]['slug'],
			'post_type'   => $args[3]['post_type'],
			'post_status' => $args[3]['post_status'],
			'numberposts' => 1
		);

		$post = get_post(get_posts($p)[0]->ID, ARRAY_A);
	...
	public function wp_getPosts( $args ) {
        ...
        $query['tax_query'] = array(array('taxonomy' => ($filter['post_type']=='songs' ? 'songs_cat' : 'videos_cat'),
                                'field' => 'slug',
	...
	public function wp_getPostFor($args){
		$username = $args[1];
		$password = $args[2];
		if ( ! $user = $this->login( $username, $password ) )
			return $this->error;

		$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'wp.getPostFor' );
		do_action( 'xmlrpc_call', 'wp.getPostFor' );
		global $wpdb;
		$postIds = $wpdb->get_results("SELECT ID FROM " . $wpdb->prefix . "posts Where ID > {$args[3]["ID"]} and post_type='songs' and post_status='publish' limit {$args[3]["post"]},{$args[3]["offset"]}" ,OBJECT);
		$struct = array();
		foreach ( $postIds as $post ) {
			$struct[] = $this->_prepare_post( get_post($post->ID, ARRAY_A), $fields );
		}
		return $struct;
	}


	public function wp_getMyPosts($args){
		$username = $args[1];
		$password = $args[2];
			/** This action is documented in wp-includes/class-wp-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'wp.getMyPosts' );
		...
		$p = array(
			'post_type'   => $args[3]['post_type'],
			'post_status' => $args[3]['post_status'],
			'numberposts' => $args[3]['numberposts'],
			'offset'=>$args[3]['offset']
		);

		$struct = array();
		$posts_list = get_posts($p);
		foreach ( $posts_list as $post ) {
			$struct[] = $this->_prepare_post( get_post($post->ID, ARRAY_A), $fields );
		}
		return $struct;
